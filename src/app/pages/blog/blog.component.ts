...
export class BlogComponent implements OnInit {
  ...
  constructor(private router: Router, private databaseHandlerService: DatabaseHandlerService, private store: Store<fromApp.AppState>) { }

  ngOnInit(): void {
    this.calculateBreakPoint(window.innerWidth);
    this.fetchData(null);
    this.store.select('auth')
      .pipe(take(1), map(state => state.user))
